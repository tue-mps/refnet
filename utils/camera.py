...
class Camera:
    def __init__(self, Intrinsic_Filename=None, Extrinsic_Filename=None, intrinsics=[], dist_coeffs=[], tVec=[],
                 rVec=[], ImageWidth=None, ImageHeight=None):

        self.MAX_ITER_NEWTON = 20
        self.MIN_EPS = 0.001

        if (Intrinsic_Filename != None and Extrinsic_Filename != None):
            self.__LoadIntrinsics_from_opcv_file(Intrinsic_Filename)
            self.__LoadExtrinsics_from_opcv_file(Extrinsic_Filename)
        elif (len(intrinsics) > 0 and len(dist_coeffs) > 0 and len(tVec) > 0 and len(rVec) > 0):
            self.ImageWidth = ImageWidth
            self.ImageHeight = ImageHeight
            self.camera_matrix = np.asarray(intrinsics)
            self.distortion_coefficients = np.asarray(dist_coeffs)
            self.rotation_matrix = cv2.Rodrigues(rVec)[0]
            self.translation_vector = np.asarray(tVec)
    ...
    def __LoadIntrinsics_from_opcv_file(self, filename):
    ...
    def __LoadExtrinsics_from_opcv_file(self, filename):
    ...
    def __worldToCam(self, x_m, y_m, z_m):
        xc_m = self.rotation_matrix[0, 0] * x_m + self.rotation_matrix[0, 1] * y_m + self.rotation_matrix[0, 2] * z_m + \
               self.translation_vector[0]

        yc_m = self.rotation_matrix[1, 0] * x_m + self.rotation_matrix[1, 1] * y_m + self.rotation_matrix[1, 2] * z_m + \
               self.translation_vector[1]

        zc_m = self.rotation_matrix[2, 0] * x_m + self.rotation_matrix[2, 1] * y_m + self.rotation_matrix[2, 2] * z_m + \
    ...
    def __UndistortedToDistorted(self, xp, yp):
        ...
        rp2 = xp * xp + yp * yp
        xs = xp * (1 + self.distortion_coefficients[0] * rp2 + self.distortion_coefficients[1] * rp2 * rp2 +
                   self.distortion_coefficients[2] * rp2 * rp2 * rp2) + 2 * self.distortion_coefficients[3] * xp * yp + \
             self.distortion_coefficients[4] * (rp2 + 2 * xp * xp)

        ys = yp * (1 + self.distortion_coefficients[0] * rp2 + self.distortion_coefficients[1] * rp2 * rp2 +
                   self.distortion_coefficients[2] * rp2 * rp2 * rp2) + 2 * self.distortion_coefficients[4] * xp * yp + \
             self.distortion_coefficients[3] * (rp2 + 2 * yp * yp)
    ...
    def worldToImage_opcv(self, x, y, z):
        ...
        imgpts, _ = cv2.projectPoints(world_points, self.rotation_matrix, self.translation_vector, self.camera_matrix,
    ...
    def UpdateModel_CameraPitch(self, dpitch_rad):
        ...
        new_rot[0, 0] = self.rotation_matrix[0, 0] * c - self.rotation_matrix[0, 2] * s
        new_rot[0, 1] = self.rotation_matrix[0, 1]
        new_rot[0, 2] = self.rotation_matrix[0, 0] * s + self.rotation_matrix[0, 2] * c

        new_rot[1, 0] = self.rotation_matrix[1, 0] * c - self.rotation_matrix[1, 2] * s
        new_rot[1, 1] = self.rotation_matrix[1, 1]
        new_rot[1, 2] = self.rotation_matrix[1, 0] * s + self.rotation_matrix[1, 2] * c

        new_rot[2, 0] = self.rotation_matrix[2, 0] * c - self.rotation_matrix[2, 2] * s
        new_rot[2, 1] = self.rotation_matrix[2, 1]
        new_rot[2, 2] = self.rotation_matrix[2, 0] * s + self.rotation_matrix[2, 2] * c

        self.rotation_matrix = new_rot
...
if __name__=='__main__':
    ...
    ImageWidth = 1920
    ImageHeight = 1080
    CAM = Camera(intrinsics=camera_matrix, dist_coeffs=dist_coeffs, tVec=tvecs, rVec=rvecs, ImageWidth=1920,
                 ImageHeight=1080)
    u = 1137
    v = 544
    x = 3.867
    y = 33.778
    z = 1.45
    x_m, _ = CAM.imageToWorld_Z(u_px=u, v_px=v, z_m=0)
    _, y_m = CAM.imageToWorld_Z(u_px=u, v_px=v, z_m=1.45)
    u,v = CAM.worldToImage_opcv(x, y, z)
    print("")
