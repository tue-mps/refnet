...
class Detection_Header(nn.Module):
    ...
    def __init__(self,config, use_bn=True, reg_layer=2):
        ...
        if config['model']['DetectionHead']=='True':
            self.conv1 = conv3x3(768, 144, bias=bias)
...
class Bottleneck_cam(nn.Module):
    expansion = 4

    def __init__(self, in_channels, out_channels, i_downsample=None, stride=1):
        super(Bottleneck_cam, self).__init__()
...
class ResNet_enc_original(nn.Module):
    def __init__(self, layer_list, num_classes, num_channels=3):
        super(ResNet_enc_original, self).__init__()
        self.in_channels = 64
        ...
        self.layer1 = self._make_layer(Bottleneck_cam, layer_list[0], planes=64)
        self.layer2 = self._make_layer(Bottleneck_cam, layer_list[1], planes=128, stride=2)
        self.layer3 = self._make_layer(Bottleneck_cam, layer_list[2], planes=256, stride=2)
        self.layer4 = self._make_layer(Bottleneck_cam, layer_list[3], planes=512, stride=2)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * Bottleneck_cam.expansion, num_classes)
    ...
    def _make_layer(self, Bottleneck_cam, blocks, planes, stride=1):
        ii_downsample = None
        layers = []

        if stride != 1 or self.in_channels != planes * Bottleneck_cam.expansion:
            ii_downsample = nn.Sequential(
                nn.Conv2d(self.in_channels, planes * Bottleneck_cam.expansion, kernel_size=1, stride=stride),
                nn.BatchNorm2d(planes * Bottleneck_cam.expansion)
            )

        layers.append(Bottleneck_cam(self.in_channels, planes, i_downsample=ii_downsample, stride=stride))
        self.in_channels = planes * Bottleneck_cam.expansion

        for i in range(blocks - 1):
            layers.append(Bottleneck_cam(self.in_channels, planes))
...
class BEV_decoder(nn.Module):
    def __init__(self, ):
        ...
        self.up1 = (UpScaling(2048, 1024))
        self.up2 = (UpScaling(1024, 512))
...
class PolarSegFusionNet_BEV(nn.Module):
    def __init__(self, blocks):
        ...
        blocks = [3, 4, 6, 3]
        self.FPN = ResNet_enc_original(layer_list=blocks, num_classes=1, num_channels=3)
